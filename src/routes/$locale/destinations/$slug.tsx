...
export const Route = createFileRoute("/$locale/destinations/$slug")({
  loader: async ({ params }) => {
    ...
    return { destination, tours: await toursForDestination({ data: destination.id }) };
  },
...
function DestinationDetail() {
  ...
  return (
    <article>
      ...
      <div className="mx-auto max-w-3xl px-4 py-10"><p className="text-lg text-muted">{field(destination, locale, "excerpt")}</p><p className="mt-4 whitespace-pre-line">{field(destination, locale, "body")}</p>{destination.languages ? <p className="mt-6 text-sm text-muted">{destination.languages} · {destination.currency}</p> : null}</div>
      {tours.length ? <div className="mx-auto max-w-6xl px-4 pb-16"><h2 className="mb-6 font-display text-3xl">{t("journeys")}</h2><div className="grid gap-6 md:grid-cols-3">{tours.map((tour) => <TourCard key={tour.id} tour={tour} />)}</div></div> : null}
    </article>
  );
}
