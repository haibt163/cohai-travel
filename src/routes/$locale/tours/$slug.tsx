...
function TourDetail() {
  ...
  const visibleDepartures = departures.slice(0, 6);
  const availableDepartures = departures.filter((departure) => departure.max_people - departure.booked > 0);
      ...
      <section className="relative min-h-[64vh] overflow-hidden bg-ink">
        ...
        <div className="relative mx-auto flex min-h-[64vh] max-w-7xl items-end px-4 pb-10 sm:px-6 md:pb-14">
          <div className="max-w-3xl text-paper">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3 text-sm text-paper-2"><span className="inline-flex items-center gap-2"><Clock3 className="size-4" />{tour.duration_days} {t("days")}</span><span className="size-1 rounded-full bg-paper/40" /><span className="inline-flex items-center gap-2"><CalendarDays className="size-4" />{availableDepartures.length} {locale === "vn" ? "ngày còn chỗ" : availableDepartures.length === 1 ? "available date" : "available dates"}</span></div>
          </div>
        </div>
      </section>
