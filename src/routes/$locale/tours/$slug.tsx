...
export const Route = createFileRoute("/$locale/tours/$slug")({
  ...
  head: ({ loaderData, params }) =>
    loaderData
      ? seoHead({
          locale: params.locale === "vn" ? "vn" : "en",
          title: `${params.locale === "vn" ? loaderData.tour.title_vn : loaderData.tour.title_en} | CoHai Travel`,
          description: params.locale === "vn" ? loaderData.tour.excerpt_vn : loaderData.tour.excerpt_en,
          pathname: `/${params.locale}/tours/${loaderData.tour.slug}`,
          alternatePathname: (locale) => `/${locale}/tours/${loaderData.tour.slug}`,
  ...
  return (
    <article>
      <div className="relative h-cover overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 mx-auto max-w-6xl px-4 pb-8 text-paper">
          <p className="text-xs uppercase tracking-caps">
            <Link to="/$locale/destinations/$slug" params={{ locale, slug: tour.dest_slug }} className="underline-offset-4 hover:underline">{field(tour, locale, "dest_title")}</Link>{" "}
      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-12 lg:grid-cols-5">
        ...
        <div className="lg:col-span-2"><BookingForm kind="tour" itemId={tour.id} departures={departures} unitPrice={tour.from_price} /></div>
      </div>
    </article>
  );
}
