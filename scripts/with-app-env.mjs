 ...
 *
 * `dev`, `build` and `preview` route Vite through this wrapper so the
 * workspace's generated VITE_* flags are applied consistently.
 */
...
function resolveCommand(command, root) {
  ...
  // Do not spawn the npm-generated `vite.cmd` shim directly. Node 24 on
  // Windows can reject `.cmd` files with EINVAL, while spawning the Vite JS
  // entrypoint through the current Node executable is stable and shell-free.
  return {
    executable: process.execPath,
    prefixArgs: [resolve(root, "node_modules", "vite", "bin", "vite.js")],
  };
}
...
function main(argv) {
  ...
  const { executable, prefixArgs } = resolveCommand(command, root);
